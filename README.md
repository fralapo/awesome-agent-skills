...
| [`frutiger-aero-site-builder`](./skills/frutiger-aero-site-builder/) | Build **Frutiger Aero** websites from scratch — the glossy 2000s Vista/iPod-era glass UI, with aurora backgrounds, skeumorphic 3D elements, asymmetric `70px 80px 0 0` border-radius, Vista metallic title bars, and categorized sidebar nav. **6 variants** (Bright Aero / Dark Aero / Frutiger Eco / Frutiger Aurora / Helvetica Aqua Aero / Neo Aero modern revival) plus 2 extra themes (Cream, Peach) selectable via `data-theme=""`. Includes period-accurate **hard mid-break gradient** (`55%/55%` zero-gap) for CTAs, layered radial-gradient aurora backgrounds with slow drift, glass panels via `backdrop-filter` + inset gloss, CSS-only skeumorphic orbs. The **anti-AI-pitfalls reference** names the 10 patterns that mark output as AI-default (centered hero + 4-card grid + Inter font + smooth gradient + uniform border-radius) and lists 5 **period layout patterns** distilled from real indie sites (Pattern A container+sidebar+main, B layout+marquee, C 7.css/98.css window framework, D ON-LINE old-web, E modern desktop UI metaphor). Two working examples: Dark Aero window-chrome portfolio + Cream-palette indie cozy. `aero-tokens.css` covers color, spacing, typography, motion, elevation. WCAG AA, mobile-first across 6 breakpoints, `prefers-reduced-motion` honored. | ~40 period + indie sites (Frutiger Aero Archive, frutiger-aero.it/.org/.neocities.org, derrek.org, cinni.net + Neocities personal pages) + 6 YouTube video essays on the aesthetic + 8 published web-design skills (Refactoring UI, Bencium, Owl-Listener, claudekit, Dammyjay93, Leonxlnx, evolv3ai accessibility, Anthropic frontend-design) + web research on FA history, revival, and CSS techniques | `frutiger aero`, `aero glass`, `vista design`, `2000s website`, `glossy gradient`, `aurora background`, `skeumorphic`, `web 2.0 gloss`, `dark aero`, `frutiger eco`, `neo aero`, `period accurate`, `indie web`, `neocities revival`, `glassmorphism alternative` |
| [`social-algorithm`](./skills/social-algorithm/) | Italian-language knowledge base for **personal brand organic acquisition + Instagram algorithm 2018-2026**. Seven chapters mined from real practitioner material: vendita indiretta (4 elements — esperienza, bisogno, cammino dell'eroe, social proof prezzemolo), epochal algorithm shift (interest-based distribution, dopaminic users, -20/24% engagement drop), creator systems & delegation (revenue/hours formula vs content/hours, 3-stage delegation ladder, Inside Out / Reference / Pillar ideation, Reference Based / Author Mode / AI-assisted scripting, full-day blocking), end-to-end acquisition system (4 atti — "Ti Vedo / Ti Valido / Ti Associo" triangle, 4 Gironi anti-patterns, 5 awareness levels, 6 acquisition funnels including profile/contact-follower/ManyChat/stories/lead magnet/quiz funnel, ADV integration prospecting+retargeting), dynamic video techniques (visual elements + metaphors, framing changes, custom b-roll, trailer-style scripts), 5 scaling principles, and 2018-2026 algorithm knowledge base (3 pillar metrics — saves/watch time/shares — diagnostic matrix "molti X ma pochi Y" with fix per signal, repost strategy post 7-Aug-2025, 8 non-inflated hook patterns, era-by-era evolution). Plus glossary (~60 terms), patterns.md (33 operational techniques), cheatsheet.md (quick-reference tables). | 5 long-form video transcripts (~30k words combined) + 8-year archive of algorithm/best-practice notes (~600 dated entries) | `personal brand`, `vendita indiretta`, `algoritmo instagram`, `instagram algorithm 2026`, `organico`, `acquisizione clienti`, `social algorithm`, `funnel organico`, `salvataggi watch time`, `riconoscibilità`, `cammino dell'eroe`, `posizionamento personal brand`, `ti vedo ti valido ti associo`, `gironi infernali`, `manychat` |
...
Pick one. Replace `<skill>` with the skill name (`seedance-prompts`, `image-gen-prompts`, `awesome-readme`, `llm-wiki`, `frutiger-aero-site-builder`, or `social-algorithm`).
...
The pattern is consistent across all five skills in this repo:
