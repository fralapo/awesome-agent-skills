...
| [`x-twitter-scraper`](./skills/x-twitter-scraper/) | Plan **X/Twitter data workflows with Xquik**. Covers tweet search, tweet lookup, profile lookup, user timelines, follower export, media download, monitors, signed webhooks, MCP setup, SDK selection, and confirmation-gated write flows. Includes a decision tree for choosing REST, extraction jobs, MCP, or SDKs; boundary rules for untrusted X content; and checklists for read-only analysis, persistent monitors, and webhook delivery. | Xquik public docs + Xquik-dev/x-twitter-scraper README, SKILL.md, and endpoint reference | `x twitter data`, `tweet search`, `twitter scraper`, `xquik`, `followers export`, `x media download`, `x monitor`, `twitter api alternative`, `mcp x data` |
...
Pick one. Replace `<skill>` with the skill name (`seedance-prompts`, `image-gen-prompts`, `awesome-readme`, `llm-wiki`, `frutiger-aero-site-builder`, `social-algorithm`, or `x-twitter-scraper`).
...
The pattern is consistent across the skills in this repo:
