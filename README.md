...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/skills-5-2ea44f?style=for-the-badge" alt="Skills count">
...
Five skills. Each is a single folder with a `SKILL.md` (frontmatter + workflow) plus a `references/` directory for deep-dive material. The agent (Claude Code, or any tool that reads the same format) matches the `description` field against whatever you're asking and loads the skill only when it's relevant. No eager bloat.
...
| [`frutiger-aero-site-builder`](./skills/frutiger-aero-site-builder/) | Build production-grade **Frutiger Aero** aesthetic websites from scratch — glossy 2000s Vista/iPod-era glass UI, aurora backgrounds, skeumorphic 3D elements, asymmetric `70px 80px 0 0` border-radius, Vista metallic title bars, sidebar nav with categorized sections. Covers **6 variants** (Bright Aero / Dark Aero / Frutiger Eco / Frutiger Aurora / Helvetica Aqua Aero / Neo Aero modern revival) + 2 extra themes (Cream warm indie, Peach cinni-derived) selectable via `data-theme=""`. Period-accurate **hard mid-break gradient pattern** (`55%/55%` zero-gap) for CTAs, layered radial-gradient aurora backgrounds with slow drift, glass panels with `backdrop-filter` + inset gloss, skeumorphic orbs (CSS-only). Dedicated **anti-AI-pitfalls reference** documenting the 10 patterns that make output look AI-default (centered hero + 4-card grid + Inter font + smooth gradient + uniform border-radius), plus 5 **period layout patterns** extracted from 41 real period + indie sites (Pattern A container+sidebar+main, Pattern B layout+marquee, Pattern C 7.css/98.css window framework, Pattern D ON-LINE old-web, Pattern E modern desktop UI metaphor). Two working examples — Dark Aero window-chrome portfolio + Cream-palette Neocities indie cozy. Ships ready-to-copy `aero-tokens.css` with full color/spacing/typography/motion/elevation token system, WCAG AA accessibility baseline, mobile-first responsive across 6 tiers, `prefers-reduced-motion` respected throughout. | 41 real period + indie sites in WSL `siti-oldweb/` (frutigeraeroarchive.org, frutiger-aero.it, frutiger-aero.neocities.org, derrek.org, cinni.net, skeltrr, tackyvillain, liashess.ch, sharyap.com + 32 others) + 6 YouTube videos (Aesthety / Musta / Carson / kandiwuff / Polygon Donut / Pelle Sjöberg — ~1M combined views) + 8 web design skills corpus (Refactoring UI, Bencium, Owl-Listener 87-skill collection, claudekit, Dammyjay93, Leonxlnx, evolv3ai accessibility, Anthropic official frontend-design) + 8 WebSearch queries on FA history/revival/CSS techniques | `frutiger aero`, `aero glass`, `vista design`, `2000s website`, `glossy gradient`, `aurora background`, `skeumorphic`, `web 2.0 gloss`, `dark aero`, `frutiger eco`, `neo aero`, `period accurate`, `indie web`, `neocities revival`, `glassmorphism alternative` |
...
Pick one. Replace `<skill>` with the skill name (`seedance-prompts`, `image-gen-prompts`, `awesome-readme`, `llm-wiki`, or `frutiger-aero-site-builder`).
