...
Seven skills. Each is a single folder with a `SKILL.md` (frontmatter + workflow) plus a `references/` directory for deep-dive material. The agent (Claude Code, or any tool that reads the same format) matches the `description` field against whatever you're asking and loads the skill only when it's relevant. No eager bloat.
...
| [`tweetclaw-openclaw`](./skills/tweetclaw-openclaw/) | Safety-reviewed OpenClaw workflow skill for **@xquik/tweetclaw**. Install and verify the TweetClaw plugin, use `explore` before live X/Twitter calls, keep credentials in OpenClaw config, separate account-backed mode from MPP read-only mode, and enforce explicit approval before posting, replying, DMs, media upload, monitors, webhooks, extraction, giveaway draws, private reads, or paid work. Includes workflow JSON shapes, approval checklist, dashboard-only boundaries, and common failure modes for agents that scrape tweets, search replies, export followers, look up users, handle media, monitor tweets, or run reviewed X automation from OpenClaw. | TweetClaw README, bundled `skills/tweetclaw/SKILL.md`, OpenClaw setup guide, agent workflow guide, npm package metadata, OpenClaw plugin docs, and Xquik public docs | `tweetclaw`, `openclaw x`, `x/twitter automation`, `scrape tweets`, `search tweets`, `search replies`, `post tweet`, `tweet reply`, `follower export`, `user lookup`, `media upload`, `media download`, `direct messages`, `monitor tweets`, `webhooks`, `giveaway draw`, `mpp` |
...
More coming when I build one that earns its place. I'd rather ship a few that work than twenty that almost do.

---

## Install

Pick one. Replace `<skill>` with the skill name (`seedance-prompts`, `image-gen-prompts`, `awesome-readme`, `llm-wiki`, `tweetclaw-openclaw`, `frutiger-aero-site-builder`, or `social-algorithm`).
...
The pattern is consistent across all skills in this repo:
